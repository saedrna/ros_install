...
""".format(name,name,name,name,name,name,name,delete_command,name)
    ...
    def create_container(self,name):
        ...
        if container_name:
            command_create_x11 = "sudo docker run --network=host -dit --name={} -v {}:{} -v /tmp/.X11-unix:/tmp/.X11-unix {} -v /dev/dri:/dev/dri {} -e DISPLAY=unix$DISPLAY -w {}  {}".format(
                    container_name,home,home,use_dri,use_snd,home,RosVersions.get_image(name))
        else:
            command_create_x11 = "sudo docker run --network=host -dit  -v {}:{} -v /tmp/.X11-unix:/tmp/.X11-unix {} -v /dev/dri:/dev/dri {} -e DISPLAY=unix$DISPLAY -w {}  {}".format(
